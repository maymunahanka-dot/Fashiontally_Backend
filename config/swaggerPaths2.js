...
const orderPaths = {
  '/api/order/list': { get: { summary: 'Get all orders for authenticated user', tags: ['Order'], security: [{ bearerAuth: [] }], description: 'Returns records from fashiontally_designs where type = "order".', responses: { 200: { description: 'Array of orders sorted by createdAt desc' }, 401: { description: 'No token' } } } },
  ...
  '/api/order/create': {
    post: {
      ...
      description: 'Saves to fashiontally_designs collection with type: "order". Matches mm NewOrderPanel format.',
      requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['name', 'orderType'], properties: {
        name:               { type: 'string', example: 'Elegant Wedding Gown' },
        orderType:          { type: 'string', example: 'wedding_gown' },
        garmentType:        { type: 'string', example: 'Dresses' },
        garmentDescription: { type: 'string', example: 'Elegant Wedding Gown' },
        ...
        fabric:             { type: 'string', example: 'silk' },
        materials:          { type: 'array', items: { type: 'string' }, example: ['silk'] },
        color:              { type: 'string', example: 'ivory' },
        quantity:           { type: 'number', example: 1 },
        measurements:       { type: 'object', example: { Bust: 36, Waist: 28, Length: 60 } },
        price:              { type: 'number', example: 80000 },
        basePrice:          { type: 'number', example: 50000 },
        deposit:            { type: 'number', example: 25000 },
        depositPaid:        { type: 'number', example: 25000 },
        balance:            { type: 'number', example: 55000 },
        balanceDue:         { type: 'number', example: 55000 },
        paymentStatus:      { type: 'string', example: 'partial' },
        additionalItems:    { type: 'array', items: { type: 'object', properties: { name: { type: 'string' }, price: { type: 'number' } } } },
        specialInstructions:{ type: 'string' },
        notes:              { type: 'string' },
        dueDate:            { type: 'string', example: '2025-09-01' },
        orderDate:          { type: 'string', example: '2025-09-01' },
  ...
  '/api/order/edit/{id}': { put: { summary: 'Edit order', tags: ['Order'], security: [{ bearerAuth: [] }], parameters: [{ in: 'path', name: 'id', required: true, schema: { type: 'string' } }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { status: { type: 'string' }, price: { type: 'number' }, paymentStatus: { type: 'string' } } } } } }, responses: { 200: { description: 'Updated' }, 404: { description: 'Not found' } } } },
